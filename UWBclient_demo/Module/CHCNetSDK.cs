...
namespace UWBclient_demo
{
    ...
    public class CHCNetSDK
    {
        ...
        ******************************************************************************/
        ...
        {
            public uint dwSize;//结构体大小
            public byte byFocusMode;//聚焦模式
            public byte byAutoFocusMode;//自动聚焦模式
            public ushort wMinFocusDistance;//最小聚焦距离
            public byte byZoomSpeedLevel;//变倍速度
            public byte byFocusSpeedLevel;//聚焦速度
            public byte byOpticalZoom;//光学变倍
            public byte byDigtitalZoom;//数字变倍
            public float fOpticalZoomLevel;//光学变倍(倍率值)
            public uint dwFocusPos;//focus值（聚焦值）
            [MarshalAsAttribute(UnmanagedType.ByValArray, SizeConst = 56, ArraySubType = UnmanagedType.I1)]
            public byte[] byRes;
        ...
        //设置设置SDK本地参数
        public enum NET_SDK_LOCAL_CFG_TYPE//本地参数配置类型//++
        {
        ...
        [DllImportAttribute(@"..\bin\HCNetSDK.dll")]
        public static extern bool NET_DVR_RemoteControl(int lUserID, uint dwCommand, IntPtr lpInBuffer, uint dwInBufferSizef);//远程控制//++

        public const uint NET_DVR_CHECK_USER_STATUS = 20005;//检测设备是否在线//++
    }
}
